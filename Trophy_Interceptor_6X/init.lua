...
function ENT:Use(activator, caller)
    if IsValid(activator) and activator:IsPlayer() then
        ...
        table.insert(activator.APS_Queue, "trophy_interceptor_6x")
        
        activator:EmitSound("items/ammo_pickup.wav", 80, 100)
        activator:ChatPrint("Picked up 6X APS. T" .. #activator.APS_Queue)
...
function ENT:Intercept(target)
    ...
    self.InterceptCount = self.InterceptCount + 1

    if self.InterceptCount >= MAX_INTERCEPTIONS then self:SelfDestruct(true) else
        self.IsLocked = false self.LockedTarget = nil
    end
end
